...
const ws = wb.addWorksheet('BLAKER77')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Blaker77/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const startMakeup = allMakeup.hasOwnProperty("blaker77") ? allMakeup["blaker77"] : 0
    ...
    const blaker77Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.20).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "BLAKER77") && headfees.hasOwnProperty("BLAKER77")) ? parseFloat(headfees["BLAKER77"] * playerCount.find((x) => x.agentName === "BLAKER77")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/blaker77.xlsx', (err, stat) => {
        res.download('./uploads/blaker77.xlsx', (err) => {
            fsExtra.removeSync("./uploads/blaker77.xlsx")
