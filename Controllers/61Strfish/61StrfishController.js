...
const ws = wb.addWorksheet('61STRFISH')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/61Strfish/db.json')

exports.getIndex = (req,res) => {
    res.render("61strfish")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("61strfish") ? allMakeup["61strfish"] : 0
    ...
    const strfish61Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "STARFISH") && headfees.hasOwnProperty("61STRFISH")) ? parseFloat(headfees["61STRFISH"] * playerCount.find((x) => x.agentName === "STARFISH")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/61strfish.xlsx', (err, stat) => {
        res.download('./uploads/61strfish.xlsx', (err) => {
            fsExtra.removeSync("./uploads/61strfish.xlsx")
