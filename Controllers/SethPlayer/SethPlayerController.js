...
const ws = wb.addWorksheet('SETHPLAYER')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/SethPlayer/db.json')

exports.getIndex = (req,res) => {
    res.render("sethplayer")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("sethplayer") ? allMakeup["sethplayer"] : 0
    ...
    const sethPlayerTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "SETHPLAYER") && headfees.hasOwnProperty("SETHPLAYER")) ? parseFloat(headfees["SETHPLAYER"] * playerCount.find((x) => x.agentName === "SETHPLAYER")["playersCount"]).toFixed(2) : 0
    ...
    const data = {
        ...
        "sethPlayerTotal": sethPlayerTotal,
        "makeUp": makeUp,
        "headFees": headFees,
    ...
    wb.write('./uploads/sethplayer.xlsx', (err, stat) => {
        res.download('./uploads/sethplayer.xlsx', (err) => {
            fsExtra.removeSync("./uploads/sethplayer.xlsx")
