...
const ws = wb.addWorksheet('21STRFISH')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/21Strfish/db.json')

exports.getIndex = (req,res) => {
    res.render("21strfish")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("21strfish") ? allMakeup["21strfish"] : 0
    ...
    const strfish21Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "STARFISH") && headfees.hasOwnProperty("21STRFISH")) ? parseFloat(headfees["21STRFISH"] * playerCount.find((x) => x.agentName === "STARFISH")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/21strfish.xlsx', (err, stat) => {
        res.download('./uploads/21strfish.xlsx', (err) => {
            fsExtra.removeSync("./uploads/21strfish.xlsx")
