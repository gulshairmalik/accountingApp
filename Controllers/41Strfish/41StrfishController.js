...
const ws = wb.addWorksheet('41STRFISH')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/41Strfish/db.json')

exports.getIndex = (req,res) => {
    res.render("41strfish")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("41strfish") ? allMakeup["41strfish"] : 0
    ...
    const strfish41Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "STARFISH") && headfees.hasOwnProperty("41STRFISH")) ? parseFloat(headfees["41STRFISH"] * playerCount.find((x) => x.agentName === "STARFISH")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/41strfish.xlsx', (err, stat) => {
        res.download('./uploads/41strfish.xlsx', (err) => {
            fsExtra.removeSync("./uploads/41strfish.xlsx")
