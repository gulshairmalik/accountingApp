...
const ws = wb.addWorksheet('COSTAAA')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Costaaa/db.json')
...
exports.getCalculatedData = (req,res) => {
  ...
  const startMakeup = allMakeup.hasOwnProperty("costaaa") ? allMakeup["costaaa"] : 0
  ...
  const scooterTotal = totalAmountOfWeek > 0 ? ((totalAmountOfWeek * 0.85) + Math.abs(lastWeekMakeUp)) : totalEarned
  const costaaaTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.15).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "COSTAAA") && headfees.hasOwnProperty("COSTAAA")) ? parseFloat(headfees["COSTAAA"] * playerCount.find((x) => x.agentName === "COSTAAA")["playersCount"]).toFixed(2) : 0
  ...
  dataToBeWrittenToExcel.forEach( record => {
    ...
    Object.keys(record ).forEach(columnName => {
      if (columnIndex !== 1) {
        if (record[columnName] >= 0) ws.cell(rowIndex, columnIndex).style({ font: { color: "#2a753e" } })
  ...
  wb.write('./uploads/costaaa.xlsx', (err, stat) => {
    res.download('./uploads/costaaa.xlsx', (err) => {
      fsExtra.removeSync("./uploads/costaaa.xlsx")
