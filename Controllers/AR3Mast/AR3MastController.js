...
const ws = wb.addWorksheet('AR3MAST')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/AR3Mast/db.json')
...
exports.getCalculatedData = (req,res) => {
  ...
  const startMakeup = allMakeup.hasOwnProperty("ar3mast") ? allMakeup["ar3mast"] : 0
  ...
  const ar3MastTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.35).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "AR3MAST") && headfees.hasOwnProperty("AR3MAST")) ? parseFloat(headfees["AR3MAST"] * playerCount.find((x) => x.agentName === "AR3MAST")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/ar3mast.xlsx', (err, stat) => {
    res.download('./uploads/ar3mast.xlsx', (err) => {
      fsExtra.removeSync("./uploads/ar3mast.xlsx")
