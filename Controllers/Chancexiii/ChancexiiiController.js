...
const ws = wb.addWorksheet('CHANCEXIII')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Chancexiii/db.json')

exports.getIndex = (req,res) => {
    res.render("chancexiii")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("chancexiii") ? allMakeup["chancexiii"] : 0
    ...
    const chancexiiiTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "CHANCEXIII") && headfees.hasOwnProperty("CHANCEXIII")) ? parseFloat(headfees["CHANCEXIII"] * playerCount.find((x) => x.agentName === "CHANCEXIII")["playersCount"]).toFixed(2) : 0
    ...
    const data = {
        ...
        "chancexiiiTotal": chancexiiiTotal,
        "makeUp": makeUp,
        "headFees": headFees,
    ...
    wb.write('./uploads/chancexiii.xlsx', (err, stat) => {
        res.download('./uploads/chancexiii.xlsx', (err) => {
            fsExtra.removeSync("./uploads/chancexiii.xlsx")
