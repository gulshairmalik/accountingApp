...
const ws = wb.addWorksheet('ARKRICK')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Arkrick/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const headFees = (playerCount.find((x) => x.agentName === "ARKRICK") && headfees.hasOwnProperty("ARKRICK")) ? parseFloat(headfees["ARKRICK"] * playerCount.find((x) => x.agentName === "ARKRICK")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/arkrick.xlsx', (err, stat) => {
        res.download('./uploads/arkrick.xlsx', (err) => {
            fsExtra.removeSync("./uploads/arkrick.xlsx")
