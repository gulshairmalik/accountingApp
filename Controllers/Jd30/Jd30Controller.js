...
const ws = wb.addWorksheet('JD30')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Jd30/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const startMakeup = allMakeup.hasOwnProperty("jd30") ? allMakeup["jd30"] : 0
    ...
    const jd30Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "JD30") && headfees.hasOwnProperty("JD30")) ? parseFloat(headfees["JD30"] * playerCount.find((x) => x.agentName === "JD30")["playersCount"]).toFixed(2) : 0
    ...
    dataToBeWrittenToExcel.forEach( record => {
        ...
        Object.keys(record ).forEach(columnName => {
            ...
            if (columnName !== "date") ws.cell(rowIndex,columnIndex++).number(parseInt(record[columnName]))
            else ws.cell(rowIndex,columnIndex++).string(record[columnName])
        })
        rowIndex++
    }) 

    wb.write('./uploads/jd30.xlsx', (err, stat) => {
        res.download('./uploads/jd30.xlsx', (err) => {
            fsExtra.removeSync("./uploads/jd30.xlsx")
