...
const ws = wb.addWorksheet('HAYESMAST')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Hayesmast/db.json')

exports.getIndex = (req,res) => {
  res.render("hayesmast")
}

exports.getCalculatedData = (req,res) => {

  const startMakeup = allMakeup.hasOwnProperty("hayesmast") ? allMakeup["hayesmast"] : 0
  ...
  const scooterTotal = totalAmountOfWeek > 0 ? ((totalAmountOfWeek * 0.65) + Math.abs(lastWeekMakeUp)).toFixed(2) : totalEarned
  const hayesMastTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.35).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "HAYESMAST") && headfees.hasOwnProperty("HAYESMAST")) ? parseFloat(headfees["HAYESMAST"] * playerCount.find((x) => x.agentName === "HAYESMAST")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/hayesmast.xlsx', (err, stat) => {
    res.download('./uploads/hayesmast.xlsx', (err) => {
      fsExtra.removeSync("./uploads/hayesmast.xlsx")
