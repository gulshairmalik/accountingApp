...
const ws = wb.addWorksheet('JOEYSPOTSA')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Joeyspotsa/db.json')

exports.getIndex = (req,res) => {
    res.render("joeyspotsa")
}
...
exports.getCalculatedData = (req,res) => {
    ...
    const joeyspotsaTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.25).toFixed(2) : 0
    const headFees = (playerCount.find((x) => x.agentName === "JOEYSPOTSA") && headfees.hasOwnProperty("JOEYSPOTSA")) ? parseFloat(headfees["JOEYSPOTSA"] * playerCount.find((x) => x.agentName === "JOEYSPOTSA")["playersCount"]).toFixed(2) : 0
    ...
    const data = {
        ...
        "joeyspotsaTotal": joeyspotsaTotal,
        "headFees": headFees,
    ...
    wb.write('./uploads/joeyspotsa.xlsx', (err, stat) => {
        res.download('./uploads/joeyspotsa.xlsx', (err) => {
            fsExtra.removeSync("./uploads/joeyspotsa.xlsx")
