...
const ws = wb.addWorksheet('JACKHIGH1')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Jackhigh1/db.json')

exports.getIndex = (req,res) => {
  res.render("jackhigh1")
}

exports.getCalculatedData = (req,res) => {

  const startMakeup = allMakeup.hasOwnProperty("jackhigh1") ? allMakeup["jackhigh1"] : 0
  ...
  const jackHigh1Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.30).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "JACKHIGH1") && headfees.hasOwnProperty("JACKHIGH1")) ? parseFloat(headfees["JACKHIGH1"] * playerCount.find((x) => x.agentName === "JACKHIGH1")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/jackhigh1.xlsx', (err, stat) => {
    res.download('./uploads/jackhigh1.xlsx', (err) => {
      fsExtra.removeSync("./uploads/jackhigh1.xlsx")
