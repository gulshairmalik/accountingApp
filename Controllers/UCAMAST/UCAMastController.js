...
const ws = wb.addWorksheet('UCAMAST')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/UCAMAST/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const ucaMastTotal = totalAmountOfWeek / 2
    const headFees = (playerCount.find((x) => x.agentName === "UCAMAST") && headfees.hasOwnProperty("UCAMAST")) ? parseFloat(headfees["UCAMAST"] * playerCount.find((x) => x.agentName === "UCAMAST")["playersCount"]).toFixed(2) : 0
    const scooterNet = scooterTotal - headFees
    const ucaMastHF = (playerCount.find((x) => x.agentName === "UCAMAST") && headfees["AgentsPPH"].hasOwnProperty("UCAMAST")) ? parseFloat(headfees["AgentsPPH"]["UCAMAST"] * playerCount.find((x) => x.agentName === "UCAMAST")["playersCount"]).toFixed(2) : 0
    const ucaMastNet = ucaMastTotal - ucaMastHF

    const data = {
        "date": datePeriod.toString(),
        "total": totalAmountOfWeek,
        "scooterTotal": scooterTotal,
        "ucaMastTotal": ucaMastTotal,
        "ucaMastHF": ucaMastHF,
        "ucaMastNet": ucaMastNet,
        "headFees": headFees,
    ...
    wb.write('./uploads/ucamast.xlsx', (err, stat) => {
        res.download('./uploads/ucamast.xlsx', (err) => {
            fsExtra.removeSync("./uploads/ucamast.xlsx")
