...
const ws = wb.addWorksheet('ZEBRAFISH')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Zebrafish/db.json')

exports.getIndex = (req,res) => {
  res.render("zebrafish")
}

exports.getCalculatedData = (req,res) => {

  const startMakeup = allMakeup.hasOwnProperty("zebrafish") ? allMakeup["zebrafish"] : 0
  ...
  const zebrafishTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.30).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "ZEBRAFISH") && headfees.hasOwnProperty("ZEBRAFISH")) ? parseFloat(headfees["ZEBRAFISH"] * playerCount.find((x) => x.agentName === "ZEBRAFISH")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/zebrafish.xlsx', (err, stat) => {
    res.download('./uploads/zebrafish.xlsx', (err) => {
      fsExtra.removeSync("./uploads/zebrafish.xlsx")
