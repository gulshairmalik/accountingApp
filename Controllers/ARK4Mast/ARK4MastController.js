...
const ws = wb.addWorksheet('ARK4MAST')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/ARK4Mast/db.json')
...
exports.getCalculatedData = (req,res) => {
  ...
  const startMakeup = allMakeup.hasOwnProperty("ark4mast") ? allMakeup["ark4mast"] : 0
  ...
  const ark4MastTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.30).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "ARK4MAST") && headfees.hasOwnProperty("ARK4MAST")) ? parseFloat(headfees["ARK4MAST"] * playerCount.find((x) => x.agentName === "ARK4MAST")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/ark4mast.xlsx', (err, stat) => {
    res.download('./uploads/ark4mast.xlsx', (err) => {
      fsExtra.removeSync("./uploads/ark4mast.xlsx")
