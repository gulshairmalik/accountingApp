...
const ws = wb.addWorksheet('ROLLMAST')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/Rollmast/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const startMakeup = allMakeup.hasOwnProperty("rollmast") ? allMakeup["rollmast"] : 0
    ...
    const scooterTotal = totalAmountOfWeek > 0 ? ((totalAmountOfWeek * 0.50) + Math.abs(lastWeekMakeUp)).toFixed(2) : totalEarned
    const rollMastTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "ROLLMAST") && headfees.hasOwnProperty("ROLLMAST")) ? parseFloat(headfees["ROLLMAST"] * playerCount.find((x) => x.agentName === "ROLLMAST")["playersCount"]).toFixed(2) : 0
    const scooterNet = scooterTotal - headFees
    const rollMastHF = (playerCount.find((x) => x.agentName === "ROLLMAST") && headfees["AgentsPPH"].hasOwnProperty("ROLLMAST")) ? parseFloat(headfees["AgentsPPH"]["ROLLMAST"] * playerCount.find((x) => x.agentName === "ROLLMAST")["playersCount"]).toFixed(2) : 0
    const rollMastNet = rollMastTotal - rollMastHF

    const data = {
        "date": datePeriod.toString(),
        "total": totalEarned,
        "scooterTotal": scooterTotal,
        "rollMastTotal": rollMastTotal,
        "makeUp": makeUp,
        "rollMastHF": rollMastHF,
        "rollMastNet": rollMastNet,
        "headFees": headFees,
    ...
    wb.write('./uploads/rollmast.xlsx', (err, stat) => {
        res.download('./uploads/rollmast.xlsx', (err) => {
        fsExtra.removeSync("./uploads/rollmast.xlsx")
