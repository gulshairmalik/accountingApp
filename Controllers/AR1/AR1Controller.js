...
const ws = wb.addWorksheet('AR1')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/AR1/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const ar1Total = totalAmountOfWeek / 2
    const headFees = (playerCount.find((x) => x.agentName === "AR1") && headfees.hasOwnProperty("AR1")) ? parseFloat(headfees["AR1"] * playerCount.find((x) => x.agentName === "AR1")["playersCount"]).toFixed(2) : 0
    const scooterNet = scooterTotal - headFees
    const ar1HF = (playerCount.find((x) => x.agentName === "AR1") && headfees["AgentsPPH"].hasOwnProperty("AR1")) ? parseFloat(headfees["AgentsPPH"]["AR1"] * playerCount.find((x) => x.agentName === "AR1")["playersCount"]).toFixed(2) : 0
    const ar1Net = ar1Total - ar1HF
    ...
    wb.write('./uploads/ar1.xlsx', (err, stat) => {
        res.download('./uploads/ar1.xlsx', (err) => {
            fsExtra.removeSync("./uploads/ar1.xlsx")
