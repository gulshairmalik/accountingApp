...
const ws = wb.addWorksheet('DALLMA')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/DallMa/db.json')
...
exports.getCalculatedData = (req,res) => {
  ...
  const startMakeup = allMakeup.hasOwnProperty("dallma") ? allMakeup["dallma"] : 0
  ...
  const dallMaTotal = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.35).toFixed(2) : 0
  const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
  const headFees = (playerCount.find((x) => x.agentName === "DALLMA") && headfees.hasOwnProperty("DALLMA")) ? parseFloat(headfees["DALLMA"] * playerCount.find((x) => x.agentName === "DALLMA")["playersCount"]).toFixed(2) : 0
  ...
  wb.write('./uploads/dallma.xlsx', (err, stat) => {
    res.download('./uploads/dallma.xlsx', (err) => {
      fsExtra.removeSync("./uploads/dallma.xlsx")
