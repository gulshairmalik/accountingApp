...
const ws = wb.addWorksheet('31STRFISH')
const JSONdb = require('simple-json-db')
const db = new JSONdb('./Controllers/31Strfish/db.json')

exports.getIndex = (req,res) => {
    res.render("31strfish")
}

exports.getCalculatedData = (req,res) => {

    const startMakeup = allMakeup.hasOwnProperty("31strfish") ? allMakeup["31strfish"] : 0
    ...
    const strfish31Total = totalAmountOfWeek > 0 ? (totalAmountOfWeek * 0.50).toFixed(2) : 0
    const makeUp = totalAmountOfWeek > 0 ? 0 : totalAmountOfWeek
    const headFees = (playerCount.find((x) => x.agentName === "STARFISH") && headfees.hasOwnProperty("31STRFISH")) ? parseFloat(headfees["31STRFISH"] * playerCount.find((x) => x.agentName === "STARFISH")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/31strfish.xlsx', (err, stat) => {
        res.download('./uploads/31strfish.xlsx', (err) => {
            fsExtra.removeSync("./uploads/31strfish.xlsx")
