...
const db = new JSONdb('./Controllers/Charsh/db.json')
...
exports.getCalculatedData = (req,res) => {
    ...
    const headFees = (playerCount.find((x) => x.agentName === "CHARSH") && headfees.hasOwnProperty("CHARSH")) ? parseFloat(headfees["CHARSH"] * playerCount.find((x) => x.agentName === "CHARSH")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/charsh.xlsx', (err, stat) => {
        res.download('./uploads/charsh.xlsx', (err) => {
            fsExtra.removeSync("./uploads/charsh.xlsx")
