...
const ws = wb.addWorksheet('HOGS99')
...
exports.getCalculatedData = (req,res) => {
    ...
    const startMakeup = allMakeup.hasOwnProperty("hogs99") ? allMakeup["hogs99"] : 0
    ...
    const headFees = (playerCount.find((x) => x.agentName === "HOGS99") && headfees.hasOwnProperty("HOGS99")) ? parseFloat(headfees["HOGS99"] * playerCount.find((x) => x.agentName === "HOGS99")["playersCount"]).toFixed(2) : 0
    ...
    wb.write('./uploads/hogs99.xlsx', (err, stat) => {
        res.download('./uploads/hogs99.xlsx', (err) => {
            fsExtra.removeSync("./uploads/hogs99.xlsx")
